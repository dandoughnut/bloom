...
spork ~ ab4.ambience(ab.cands4, 4);
        ...
        while (true)
        {
           ...
           else // correct movement
           {
               ...
               else
               {
                   ...
                   {
                       bassNotes[mode % bassNotes.size()] + notes[ Math.floor((leftHeight - 0.1) / 0.5 * (notes.size() - 1)) $ int ] => curNote;
                       if (curNote != prevNote)
                       {
                           e.keyOff();
                           0.05::second => now;
