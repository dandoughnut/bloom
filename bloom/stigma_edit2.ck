    ...
    fun void vocalise(int notes[], int cands[], int movementCondition, float maxThreshold)
    {
        ...
        while (true)
        {
            ...
            else 
            {
                ...
                if (leftHeight < 0.1)
                {
                    <<< "in if leftheight lower than 0.1" >>>;
                    if (choirgain.gain() > 0.05)
                    {
                        muteGain(choirgain, 50);
                        <<< "mute gain" >>>;
                    }
                ...
                else // if leftHeight is not below 0.1
                {
                    <<< "in else" >>>;
                    Math.min(cands.size() - 1, ((leftHeight - 0.1) / (maxThreshold - 0.1) * cands.size())) $ int => int curpos;
                    if (choirgain.gain() < 0.05)
                    {
                        <<< "adjusting gain" >>>;
                        adjustGain(choirgain, 1.0, 50);
                    }
