...
SinOsc osc => ADSR env => dac; env => NRev reverb => dac; 0.25 => osc.gain; (1::ms, 100::ms, 0, 1::ms) => env.set; 0.1 => reverb.mix; "224.0.0.1" => string hostname; 6449 => int port; OscIn oscIn; port => oscIn.port; OscMsg oscMsg; oscIn.addAddress("/oscCom"); 0 => int mode; 1 => int movement; 60 => int noteOSC; -1 => int prevNoteOSC; float leftHeight; float leftX; float leftY; float rightHeight; float rightX; float rightY; float leftDifVert; float rightDifVert; float leftDif; float rightDif; time lastPressed; Math.random2(0, 6) => int which; string instrument[7]; "boyschoir.wav" => instrument[0]; "cello.wav" => instrument[1]; "cinema.wav" => instrument[2]; "horn.wav" => instrument[3]; "japanflute.wav" => instrument[4]; "mixchoir.wav" => instrument[5]; "naflute.wav" => instrument[6]; fun void oscRcv() { while (true) { <<< "waiting for an OSC message..." >>>; oscIn => now; while (oscIn.recv(oscMsg) != 0) { oscMsg.getInt(0) => movement; oscMsg.getInt(1) => mode; oscMsg.getInt(2) => noteOSC; <<< "movement and mode", movement, mode >>>; 100::ms => now; } } } KBHit kb; 0 => float DEADZONE; 0 => int device; if( me.args() ) me.arg(0) => Std.atoi => device; Hid trak; HidMsg msg; if( !trak.openJoystick( device ) ) me.exit(); <<< "joystick '" + trak.name() + "' ready", "" >>>; class GameTrak { time lastTime; time currTime; float lastAxis[6]; float axis[6]; } fun void keyboardctrl() { while( true ) { kb => now; <<< "controlling with keyboard" >>>; while( kb.more() ) { kb.getchar() => int c; if (c == 119) { if (movement < 3) movement++; 0 => mode; } else if (c == 115)  { if (movement > 1) movement--; 0 => mode; } else if (c == 97) mode--; else if (c == 100) mode++; <<< "ascii:", c >>>; <<< "movement", movement, "mode", mode >>>; } 0.05::second => now; } } GameTrak gt; Ambience ab; Ambience ab2; Ambience ab3; Arpeggiator arp; Rain rn; Wind wn; Xylo xy; Finale fin; Finale fin2; spork ~ gametrak(); spork ~ oscRcv(); spork ~ keyboardctrl(); spork ~ ab.ambience(ab.cands1, 1); spork ~ ab2.ambience(ab.cands2, 2); spork ~ ab3.ambience(ab.cands3, 3); spork ~ arp.arpeggiate(arp.notes1, arp.bassNotes1, 2); spork ~ xy.starlight(3); spork ~ wn.windblows(3);spork ~ rn.ctrlRain(3); spork ~ fin.finale(); spork ~ fin2.lastNote(); while( true ) { <<< leftHeight, rightHeight, leftDif, rightDif >>>; gt.axis[5] => rightHeight; gt.axis[2] => leftHeight; Math.sqrt(Math.pow((gt.axis[0]-gt.lastAxis[0]), 2) + Math.pow((gt.axis[1]-gt.lastAxis[1]), 2)) => leftDif; Math.sqrt(Math.pow((gt.axis[3]-gt.lastAxis[3]), 2) + Math.pow((gt.axis[4]-gt.lastAxis[4]), 2)) => rightDif;
Math.sqrt(Math.pow((gt.axis[5]-gt.lastAxis[5]), 2)) => rightDifVert; 100::ms => now;}
...
class Finale
{
    SndBuf inst => ADSR finAdsr => NRev finRev; 
    finRev=> ResonZ finRes => Gain finGain => dac;
    finAdsr.set(10::ms, 100::ms, 0.99, 50::ms);
    0.1 => finRev.mix; 0 => finGain.gain;
    1 => inst.loop; -1 => int prevFinNote;
    0 => int introduced; me.dir() + instrument[which] => inst.read;
